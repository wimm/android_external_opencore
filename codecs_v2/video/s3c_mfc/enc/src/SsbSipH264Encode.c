//#define LOG_NDEBUG 0
#define LOG_TAG "SsbSipH264Encode.c"
#include <utils/Log.h>
...
int SsbSipH264EncodeExe(void *openHandle)
{
	...
	if(mfc_args.enc_exe.out_header_size > 0) 
	{
		...
		// added by RainAde for composer interface
		pCTX->enc_sps_size = mfc_args.enc_exe.out_header0_size;
		pCTX->enc_pps_size = mfc_args.enc_exe.out_header1_size;
		
		LOGV("HEADER SIZE = %d\n", pCTX->enc_hdr_size);
		LOGV("enc_sps_size  = %d\n", pCTX->enc_sps_size);
		LOGV("enc_pps_size  = %d\n", pCTX->enc_pps_size);
		
		LOGV("SPS] mfc_args.enc_exe.out_header0_size  = %d\n", mfc_args.enc_exe.out_header0_size);
		LOGV("PPS] mfc_args.enc_exe.out_header1_size  = %d\n", mfc_args.enc_exe.out_header1_size);
...
int SsbSipH264EncodeSetConfig(void *openHandle, H264_ENC_CONF conf_type, void *value)
{
	...
	case H264_ENC_SETCONF_RC_PARA:

		mfc_args.set_config.in_config_param     = MFC_SET_CONFIG_ENC_SEQ_RC_PARA;
		mfc_args.set_config.in_config_value[0]  =  *((unsigned int *) value);
		mfc_args.set_config.in_config_value[1]  = 0; // not necessary

		r = ioctl(pCTX->hOpen, IOCTL_MFC_SET_CONFIG, &mfc_args);
		if ( (r < 0) || (mfc_args.set_config.ret_code < 0) ) 
		{
			LOG_MSG(LOG_ERROR, "SsbSipH264EncodeSetConfig", "Error in H264_ENC_SETCONF_RC_PARA.\n");
			return SSBSIP_H264_ENC_RET_ERR_SETCONF_FAIL;
		}
		break;

	// RainAde : for setting Picture Quantization Step on RC disable
	case H264_ENC_SETCONF_PIC_QS:

		mfc_args.set_config.in_config_param     = MFC_SET_CONFIG_ENC_PIC_QS;
		mfc_args.set_config.in_config_value[0]  =  *((unsigned int *) value);
		mfc_args.set_config.in_config_value[1]  = 0; // not necessary
		...
		if ( (r < 0) || (mfc_args.set_config.ret_code < 0) ) 
		{
			LOG_MSG(LOG_ERROR, "SsbSipH264EncodeSetConfig", "Error in H264_ENC_SETCONF_PIC_QS.\n");
			return SSBSIP_H264_ENC_RET_ERR_SETCONF_FAIL;
		}
		break;
...
int SsbSipH264EncodeGetConfig(void *openHandle, H264_ENC_CONF conf_type, void *value)
{
	...
	// added by RainAde for composer interface
	case H264_ENC_GETCONF_SPS_SIZE:
		*((int *)value) = pCTX->enc_sps_size;
		break;
		
	case H264_ENC_GETCONF_PPS_SIZE:
		*((int *)value) = pCTX->enc_pps_size;
		break;
		
	// RainAde for Encoding pic type
	case H264_ENC_GETCONF_PIC_TYPE:
		mfc_args.get_config.in_config_param      = MFC_GET_CONFIG_ENC_PIC_TYPE;
		...
		if ((r < 0) || (mfc_args.get_config.ret_code < 0)) 
		{
			...
			return SSBSIP_H264_ENC_RET_ERR_GETCONF_FAIL;
		}
