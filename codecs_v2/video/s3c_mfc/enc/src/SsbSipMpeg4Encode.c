//#define LOG_NDEBUG 0
#define LOG_TAG "SsbSipMpeg4Encode"
#include <utils/Log.h>
...
void *SsbSipMPEG4EncodeGetInBuf(void *openHandle, long size)
{
	...
	{
		LOGV("Failed in get FRAM_BUF address\n");
...
int SsbSipMPEG4EncodeSetConfig(void *openHandle, MPEG4_ENC_CONF conf_type, void *value)
{
	...
	case MPEG4_ENC_SETCONF_RC_PARA:
		...
		if ((r < 0) || (mfc_args.set_config.ret_code < 0)) 
		{
			LOG_MSG(LOG_ERROR, "SsbSipMPEG4DecodeSetConfig", "Error in MPEG4_ENC_SETCONF_RC_PARA.\n");
			return SSBSIP_MPEG4_ENC_RET_ERR_SETCONF_FAIL;
		}
		break;

	// RainAde : for setting Picture Quantization Step on RC disable
	case MPEG4_ENC_SETCONF_PIC_QS:
		...
		if ((r < 0) || (mfc_args.set_config.ret_code < 0)) 
		{
			LOG_MSG(LOG_ERROR, "SsbSipMPEG4DecodeSetConfig", "Error in MPEG4_ENC_SETCONF_PIC_QS.\n");
			return SSBSIP_MPEG4_ENC_RET_ERR_SETCONF_FAIL;
		}
		break;
...
int SsbSipMPEG4EncodeGetConfig(void *openHandle, MPEG4_ENC_CONF conf_type, void *value)
{
	...
	// RainAde for Encoding pic type
	case MPEG4_ENC_GETCONF_PIC_TYPE:
		...
		if ((r < 0) || (mfc_args.get_config.ret_code < 0)) {
			...
			return SSBSIP_MPEG4_ENC_RET_ERR_GETCONF_FAIL;
		}
