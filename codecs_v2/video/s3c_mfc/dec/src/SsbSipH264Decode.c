...
int SsbSipH264DecodeExe(void *openHandle, long lengthBufFill)
{
	...
	if (!pCTX->fInit)
	{
		...
		{
			LOG_MSG(LOG_ERROR, "SsbSipH264DecodeExe", "IOCTL_MFC_H264_DEC_INIT fail\n");
	...
	{
		LOG_MSG(LOG_ERROR, "SsbSipH264DecodeExe", "IOCTL_MFC_H264_DEC_EXE fail\n");
...
int SsbSipH264DecodeGetConfig(void *openHandle, H264_DEC_CONF conf_type, void *value)
{
	...
	case H264_DEC_GETCONF_CROP_INFO:				
		mfc_args.get_config.in_config_param		= MFC_GET_CONFIG_DEC_H264_CROPINFO;
		mfc_args.get_config.out_config_value[0] = 0;
		mfc_args.get_config.out_config_value[1] = 0;
		...
		if ((r <0) || (mfc_args.get_config.ret_code < 0)) 
		{
			LOG_MSG(LOG_ERROR, "SsbSipH264DecodeGetConfig", "Error in H264_DEC_GETCONF_CROP_INFO.\n");
			return SSBSIP_H264_DEC_RET_ERR_GETCONF_FAIL;
		}
	
		if (mfc_args.get_config.out_config_value[0] == 0 && mfc_args.get_config.out_config_value[1] == 0) 
		{
			((SSBSIP_H264_CROP_INFO *)value)->left	= 0;
			((SSBSIP_H264_CROP_INFO *)value)->right	= pCTX->width;
			((SSBSIP_H264_CROP_INFO *)value)->top	= 0;
			((SSBSIP_H264_CROP_INFO *)value)->bottom	= pCTX->height;
		} 
		else 
		{
			((SSBSIP_H264_CROP_INFO *)value)->left	= ((mfc_args.get_config.out_config_value[0]>>10) & 0x3FF);
			((SSBSIP_H264_CROP_INFO *)value)->right	= pCTX->width - ((mfc_args.get_config.out_config_value[0] & 0x3FF));
			((SSBSIP_H264_CROP_INFO *)value)->top	= ((mfc_args.get_config.out_config_value[1]>>10) & 0x3FF);
			((SSBSIP_H264_CROP_INFO *)value)->bottom	= pCTX->height - ((mfc_args.get_config.out_config_value[1] & 0x3FF));	
		}
